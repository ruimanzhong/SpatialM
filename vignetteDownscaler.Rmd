...
The column names of `depoint` data have to be `pvalue`,`geometry`, where `pvalue` stands for the estimated value for point data and the other column includes spatial information.

The column names of `dearea` data have to be `avalue`,`geometry`, where `avalue` stands for the estimated value for areal data and the other column includes spatial information.
```{r}
# CRS projection
crsproj <- 4326
