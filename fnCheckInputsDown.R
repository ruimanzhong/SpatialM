...
fnCheckInputsDown <- function(depoint, dearea, dppoint, dparea, boundaryregion) {
  
  # Need to provide point and areal data for estimation
  if (is.null(dearea) || is.null(depoint)){
    stop("'Valid estimation data input required'")
  }
  
  # Need to provide point and/or areal data for prediction
  if (is.null(dparea) && is.null(dppoint)){
