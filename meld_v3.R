...
meld = function(formula, mesh = NULL, target, areaPre.sf = NULL,
                p.sf = NULL , area.sf = NULL,
                prior.range = c(2, 0.01), prior.sigma = c(10, 0.01), 
                proN = 4326) {
  
  check_meld(area.sf, p.sf, target, proN)
  ...
  if(is.null(area.sf) == T) { 
    ...
    prediction = geosp(formula, mesh = mesh, p.sf = p.sf, 
...
melding = function(mesh = NULL, target, areaPre.sf = NULL, formula,
                   p.sf = NULL , area.sf = NULL,
                   spde, proN = 4326){
