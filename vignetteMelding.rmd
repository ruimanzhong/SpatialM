...
packages <- c( 'viridis','rnaturalearthhires',
              'sf','INLA','rgeoboundaries','tidyverse','rnaturalearth','raster', 'ggplot2')
...
In this example, we specify the projection as 2158 or UTM 29 zone.
...
```{r warning=FALSE}
setwd("~/Documents/Project 1/resources/sptialM/data")
dearea <- st_read("dearea.shp")%>% st_transform(crsproj)
ggplot(data = boundaryregion) + geom_sf() +
...
In the previous session, we created a continuous surface called `dpcontsurface`. Now we will illustrate how to make prediction for a continuous surface. The `dpcontsurface` is a point predicted data, thus, we specify `dparea` to be NULL. The plot of predicted mean and its 95\% credible interval is generated by our function `plot_sp()`. 

```{r}
mesh <- fnCreateMesh(depoint, boundaryregion)
respre_meld <- fnPredictMelding(depoint =  depoint, dearea = dearea, dppoint = dpcontsurface, dparea = NULL, boundaryregion = boundaryregion,  mesh = mesh)

results = respre_meld[[1]]
